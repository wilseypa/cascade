...
%% We choose coalgebras over their better known duals, algebras, due
%% to the non-terminating and heterogeneous nature of the types of
%% systems we model.  Coalgebras are more natural than algebras for
%% representing non-terminating systems.  The inductive proof theory
%% associated with algebras requires a base case or initial state that
%% may not exist in many embedded systems.  As stream transformers,
%% coalgebras and their associated proof techniques are well equipped
%% to deal with reactive, non-terminating embedded systems.
...
% discussed earlier are examples of reflective Rosetta operations
% making up the \emph{facet algebra} used to compose specifications.
% Products compose specifications and functors transform
% specifications to define new specifications. Homomorphism is a
% relation over specification pairs.  Other important facet algebra
% operations include equivalence (isomorphism), relabeling and
% instantiation.
...
%% PERRY: ok, so we're mixing functors and functions in this next
%% paragraph.  i believe they should all be replaced with functors,
%% but would prefer you validate/change (especially since we have
%% time).
%% PHIL: changed the names of the functors to be functions.  They are
%% both - a functor is implemented as a function, just over models.
%% PERRY: this paragraph still mixes the terms functors and functions.  also, if we change
%% to functions, should we link functions in the paragraph below to functors in the
%% paragraph above.
